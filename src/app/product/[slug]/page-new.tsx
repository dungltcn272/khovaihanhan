...
export default function ProductDetail({ params }: PageProps) {
  ...
  // Tính tổng tiền
  const totalPrice = product ? product.pricePerMeter * quantity : 0;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="relative aspect-square rounded-lg overflow-hidden mb-4 bg-gray-100">
              ...
              {product.images.length > 1 && (
                <>
                  <button
                    onClick={prevImage}
                    className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/80 hover:bg-white rounded-full flex items-center justify-center shadow-lg transition-all duration-200 z-10"
                    aria-label="Previous image"
                  >
                    <svg className="w-6 h-6 text-[#3d3226]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
                    </svg>
                  </button>
                  <button
                    onClick={nextImage}
                    className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/80 hover:bg-white rounded-full flex items-center justify-center shadow-lg transition-all duration-200 z-10"
              ...
              {product.images.map((image, index) => (
                <button
                  ...
                  className={`relative aspect-square rounded-lg overflow-hidden ${
                    selectedImage === index ? 'ring-4 ring-[#c9a961]' : 'ring-1 ring-gray-200'
                  } hover:ring-[#c9a961] transition-all duration-200`}
                >
                  <Image
                    src={image}
          ...
          <div>
            ...
            {/* Price */}
            <div className="mb-6 p-6 bg-gradient-to-r from-[#f5f1e8] to-[#e8dcc8] rounded-xl">
            ...
            {/* Quantity Selector */}
            <div className="mb-8 p-6 bg-white rounded-xl shadow-lg border-2 border-[#e8dcc8]">
              <label className="block text-lg font-semibold text-[#3d3226] mb-4">
                Số lượng (mét):
              </label>
              <div className="flex items-center gap-4 mb-6">
                <button
                  onClick={decrementQuantity}
                  className="w-14 h-14 bg-[#c9a961] hover:bg-[#a88947] text-white rounded-lg font-bold text-2xl transition-all duration-200 transform hover:scale-105 active:scale-95 shadow-md"
                ...
                <input
                  ...
                  className="w-24 h-14 text-center text-2xl font-bold border-2 border-[#c9a961] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#c9a961]"
                  min="1"
                />
                <button
              ...
              <div className="border-t-2 border-[#e8dcc8] pt-4">
                ...
                <div className="flex justify-between items-center pt-3 border-t-2 border-[#c9a961]">
                  <span className="text-xl font-bold text-[#3d3226]">Tổng tiền:</span>
                  <span className="text-3xl font-bold text-[#c9a961]">{formatPrice(totalPrice)}</span>
                </div>
              </div>
            </div>
