...
import { getProductById, updateProduct, deleteProduct } from '@/lib/firebaseService';
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        console.error(error);
        alert('Lỗi: Không tải được sản phẩm');
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      alert('Lỗi: Không cập nhật được sản phẩm');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <h2 className="text-2xl md:text-3xl font-bold mb-6 md:mb-8">Chỉnh Sửa Sản Phẩm</h2>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg md:rounded-xl shadow p-4 md:p-8 space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Vd: Vải lụa tơ tằm cao cấp"
          />
        </div>
